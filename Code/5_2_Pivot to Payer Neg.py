...
payer_neg_path = os.path.join(folder_path, "5_2_25_Payer Neg.xlsx")
output_path = os.path.join(folder_path, "Updated_5_2_25_Payer Neg.xlsx")
...
for idx in range(1, payer_neg_df.shape[0]):
    ...
    for _, row in case_pivot_df.iterrows():
        ...
        values = row.iloc[3:7].values  # max, min, avg, median
        col_names = [f"{cpt_code} max", f"{cpt_code} min", f"{cpt_code} avg", f"{cpt_code} median"]
